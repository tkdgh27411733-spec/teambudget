...
WEB_APP_URL = st.secrets.get("GAS_URL", "https://script.google.com/macros/s/..../exec")
...
def fetch_data(url):
    ...
    try:
        ...
        if isinstance(data, dict) and "error" in data:
            st.error(f"구글 시트 에러: {data['error']} (시트 이름을 확인하세요)")
            return pd.DataFrame()
...
def submit_data(id, member, month, category, amount):
    if not url:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"❌ 저장에 실패했습니다. (상태 코드: {response.status_code})")
            return False
    except Exception as e:
        st.error(f"요청 오류: {e}")
        return False
...
with tab_input:
    ...
    with col2:
        ...
        if WEB_APP_URL:
            ...
            st.info("좌측 사이드바에 웹앱 URL을 입력해주세요.")
...
with tab_dashboard:
    if WEB_APP_URL:
        ...
        st.warning("좌측 사이드바에 웹앱 URL을 입력해야 대시보드를 볼 수 있습니다.")
